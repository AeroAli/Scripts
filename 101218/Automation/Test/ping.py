...
if __name__ == "__main__":
    for host in hosts:
        print("host: " + host + "\n")
        os.mkdir("Results/" + host)
        with open("/root/Desktop/Automation/Test/Results/" + host + "/users.txt", "w") as file:
            results[host] = {"users": enumUsers(host)}
            for index,user in enumerate(results[host]["users"]):
                file.write(str(user)  + "\n")
        with open("/root/Desktop/Automation/Test/Results/" + host + "/rid.txt", "w") as file:
            results[host] = {"users": enumUsers(host)}
            for index,user in enumerate(results[host]["users"]):
                file.write(str(results[host]["users"][user]["rid"])  + "\n")
        with open("/root/Desktop/Automation/Test/Results/" + host + "/sid.txt", "w") as file:
            results[host] = {"users": enumUsers(host)}
            for index,user in enumerate(results[host]["users"]):
        ...
        with open("/root/Desktop/Automation/Test/Results/" + host + "/rid.txt", "w") as file:
            ...
            for index,user in enumerate(results[host]["users"]):
                file.write(str(results[host]["users"][user]["sid"])  + "\n")
        with open("/root/Desktop/Automation/Test/Results/" + host + "/raw.txt", "w") as file:
            results[host] = {"users": enumUsers(host)}
            for index,user in enumerate(results[host]["users"]):
